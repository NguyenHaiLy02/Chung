    ...
    <div class="order-product-detail"style="margin: 20px">
        <div class="product-detail">
                ...
                <tbody>
                    @foreach ($orderDetails as $order)
                        <tr>
                            <td>
                                @if ($order['hinhanh'])
                                    <img src="{{ asset('storage/' . $order['hinhanh']) }}" alt="{{ $order['tenSanPham'] }}"
            ...
            <form action="{{ route('order.fromCart') }}" method="POST">
                ...
                @foreach ($orderDetails as $order)
                    <input type="hidden" name="orderDetails[{{ $loop->index }}][sanPhamId]"
                        value="{{ $order['sanPhamId'] }}">
                    <input type="hidden" name="orderDetails[{{ $loop->index }}][quantity]"
                        value="{{ $order['quantity'] }}">
                    <input type="hidden" name="orderDetails[{{ $loop->index }}][totalPrice]"
                ...
                <div>
                    ...
                    <input type="text" name="address" required placeholder="Nhập địa chỉ giao hàng"
                        style="padding: 8px; font-size: 14px; width: 100%; margin-bottom: 10px;">
                </div>
